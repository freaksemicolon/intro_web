...
async function initializeApp() {
    try {
        ...
        if (specsIdxRes.ok) {
            ...
            const specPromises = specFiles.map(async (file, index) => {
        ...
        document.getElementById('feed-content').innerHTML = `
            <div class="card" style="text-align: center; color: var(--accent-color); padding: 40px;">
                <i class="fas fa-exclamation-triangle" style="font-size: 2rem; margin-bottom: 15px; display: block;"></i>
                시스템 초기화 중 오류가 발생했습니다.<br>
                <span style="font-size: 0.9rem; color: var(--text-muted); padding-top:10px; display:block;">
                    로컬 환경(file://)에서는 보안 정책상 작동하지 않을 수 있습니다.<br>
                    <strong>VS Code의 Live Server</strong> 등을 통해 접속해 주세요.
                </span>
            </div>`;
    }
}
    ...
    if (hash === '#/' || hash === '#' || hash === '') {
        ...
        if (pathParts.length === 3 && pathParts[2]) {
            ...
            if (spec) {
                ...
                content.innerHTML = `<div class="card">스펙을 찾을 수 없습니다.</div>`;
            }
        } else {
        ...
        if (pathParts.length === 3 && pathParts[2]) {
            ...
            if (post) {
                ...
                content.innerHTML = `<div class="card">포스트를 찾을 수 없습니다.</div>`;
            }
        } else {
        ...
        content.innerHTML = `<div class="card">404 - Not Found</div>`;
    }
}
