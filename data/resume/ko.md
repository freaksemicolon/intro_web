---
name: 장석우
role: Product Engineer · Rootive CTO
email: janghh1228@gm.gist.ac.kr
phone:
location: 대한민국 광주
github: https://github.com/JangSW52
portfolio: https://www.jangsw.com/
---

## 소개

AI, 앱, 백엔드와 다양한 기술을 연결해 아이디어를 실제 제품으로 완성합니다.

## 경력

### Rootive · CTO

**2025.09 — 현재**

- 제품의 기술 방향 설계
- 모바일, 백엔드 및 AI 기능 개발 총괄
- 아이디어 검증부터 실제 제품 구현까지 개발 과정 관리

### Cybernetics Imaging Systems · 개발 인턴

**2025.12 — 2026.02**

- SPIM 현미경 실험 환경을 위한 Django 기반 제어·관리 시스템 개발
- 사내 서버 환경 구성 및 기능 검증

## 학력

### 광주과학기술원

- 전기전자컴퓨터공학과
- 2025.03 — 현재

### 창원과학고등학교

- 정보 분야
- 2022.03 — 2025.02

## 프로젝트

### 프로젝트 이름

- 기간:
- 담당 역할:
- 주요 기능:
- 사용 기술:
- 결과 및 성과:
- GitHub 또는 배포 링크:

## 기술

- 프로그래밍 언어: JavaScript, TypeScript, Python, C++
- 프레임워크: React, React Native, Django, FastAPI 등
- AI·데이터: Machine Learning, Computer Vision, MediaPipe, AI Agent
- 도구: Git, GitHub, Docker, Figma, Notion

## 대외활동 및 수상

- 이 영역을 자유롭게 수정하거나 항목을 추가하세요.

## 어학 및 자격

- 시험명, 점수, 취득일 또는 자격증 정보를 입력하세요.

## 추가 정보

- 연구, 발표, 창업, 특허, 봉사활동 등을 입력하세요.
